...
instance Monad m => BaseM m m where
  inBase = id

instance (BaseM m n, MonadT t, Monad (t m)) => BaseM (t m) n where
...
class (Monad m) => ReaderM m i where
  -- | Get the context.
  ask :: m i
...
instance (ReaderM m j, MonadT t, Monad (t m)) => ReaderM (t m) j where
  ask = lift ask



-- | Classifies monads that can collect values of type @i@.
class (Monad m) => WriterM m i where
...
instance (WriterM m j, MonadT t, Monad (t m)) => WriterM (t m) j where
  put x = lift (put x)



-- | Classifies monads that propagate a state component of type @i@.
class (Monad m) => StateM m i where
  -- | Get the state.
  get :: m i
  -- | Set the state.
  set :: i -> m ()
...
instance (StateM m j, MonadT t, Monad (t m)) => StateM (t m) j where
  get = lift get
  set x = lift (set x)
...
instance (ExceptionM m j, MonadT t, Monad (t m)) => ExceptionM (t m) j where
...
-- sub-computation.
class (ReaderM m i) => RunReaderM m i where
...
-- a sub-computation.
class WriterM m i => RunWriterM m i where
...
class ExceptionM m i => RunExceptionM m i where
